      ...
      if (res.ok) {
        ...
        if (data.models && data.models.length > 0) {
          ...
          if (!availableModels.includes(currentModel)) {
            const matchesPhi3 = availableModels.find((name: string) => name.toLowerCase().includes('phi3') || name.toLowerCase().includes('phi'));
            if (matchesPhi3) {
...
<svelte:head>
  ...
  <meta name="description" content="Dashboard d'administration TechCorp et assistant de chat financier en mode sombre." />
</svelte:head>
...
<div class="h-screen bg-zinc-950 text-zinc-50 flex overflow-hidden font-sans antialiased">
  ...
  <aside class="w-80 bg-zinc-900 border-r border-zinc-800 flex flex-col justify-between shrink-0">
    <div class="p-6 space-y-6">
      ...
      <div class="flex items-center gap-3">
        <div class="h-9 w-9 rounded-lg bg-emerald-600/10 border border-emerald-500/20 flex items-center justify-center text-emerald-400">
          <Terminal size={18} />
        </div>
        <div>
          <h2 class="text-sm font-semibold tracking-wider uppercase text-zinc-400">TechCorp Admin</h2>
          <p class="text-[10px] text-zinc-500">v2.4 - Console Décisionnelle</p>
        </div>
      </div>
      ...
      <Card class="bg-zinc-950 border-zinc-800 shadow-none">
        <CardContent class="p-4 space-y-4">
          ...
          <div class="pt-3.5 border-t border-zinc-800 flex flex-col gap-2">
            ...
            {#if localSimulationMode}
              <Badge class="bg-zinc-900 text-zinc-300 hover:bg-zinc-900 border-zinc-800 w-full justify-center py-1 text-[11px] font-mono select-none">
      ...
      <div class="space-y-4">
        <!-- API URL Config -->
        <div class="space-y-2">
          <label for="api-url" class="text-xs font-semibold text-zinc-400 flex items-center gap-1.5">
        ...
        <div class="bg-zinc-950 border border-zinc-800 rounded-xl p-3.5 flex items-center justify-between">
  ...
  <main class="flex-1 flex flex-col bg-zinc-950 overflow-hidden relative">
    <!-- Header -->
    <header class="h-16 border-b border-zinc-800 bg-zinc-900/30 backdrop-blur flex items-center justify-between px-8 z-10 shrink-0">
      <div class="flex items-center gap-3">
        <div class="h-2 h-2 w-2 rounded-full bg-emerald-500 animate-pulse"></div>
        <div>
          <h1 class="text-sm font-semibold text-zinc-150 uppercase tracking-wider">Financial Intelligence Engine</h1>
          <p class="text-[10px] text-zinc-500">Inférence temps réel & Analyses prédictives</p>
        </div>
      </div>

      <div class="flex items-center gap-4 text-xs text-zinc-400">
        <span>Hôte : <code class="bg-zinc-900 px-1.5 py-0.5 rounded text-[10px] text-zinc-300">{localSimulationMode ? 'Simulation locale' : new URL(localApiUrl).host}</code></span>
      </div>
    </header>
      ...
      <ScrollArea class="h-full w-full">
        <div class="max-w-4xl mx-auto px-6 py-8 space-y-6">
          {#if $chatStore.messages.length === 0}
            ...
            <div class="h-[60vh] flex flex-col items-center justify-center text-center max-w-md mx-auto space-y-6">
              <div class="h-14 w-14 rounded-2xl bg-zinc-900 border border-zinc-800 flex items-center justify-center text-zinc-400 shadow-md">
                <MessageSquareCode size={24} />
              </div>
              <div class="space-y-2">
                <h3 class="text-sm font-semibold text-zinc-200">Prêt pour l'inférence financière</h3>
                <p class="text-xs text-zinc-400 leading-relaxed">
                  Interrogez l'assistant décisionnel sur des analyses financières, des études de risques ou tout autre sujet. Le store gère l'état d'inférence de bout en bout.
                </p>
              </div>
              <div class="flex gap-2">
                <Button 
                  ...
                  size="sm"
                  onclick={() => {
                    inputMessage = "Simule une analyse des risques géopolitiques sur les marchés européens.";
                    handleSend();
                  }}
              ...
              {#each $chatStore.messages as message (message.id)}
                <div class="flex gap-4 {message.role === 'user' ? 'justify-end' : 'justify-start'}">
                  ...
                  {#if message.role === 'assistant'}
                    <div class="h-8 w-8 rounded-lg bg-zinc-900 border border-zinc-800 flex items-center justify-center text-zinc-300 shrink-0 shadow-sm mt-0.5">
                      <Bot size={15} class={$chatStore.isLoading && !message.content ? 'animate-spin' : ''} />
                    </div>
                  {/if}
                  ...
                  <div class="space-y-1 max-w-[78%]">
                    ...
                    -->
                    <div class="p-4 text-xs leading-relaxed whitespace-pre-wrap transition-all
                      {message.role === 'user' 
                    ...
                    <!-- Metadata -->
                    <div class="flex items-center gap-2 px-1 text-[10px] text-zinc-500" class:justify-end={message.role === 'user'}>
                  ...
                  {#if message.role === 'user'}
                    <div class="h-8 w-8 rounded-lg bg-emerald-950 border border-emerald-800 text-emerald-400 flex items-center justify-center shrink-0 shadow-sm mt-0.5">
                      <User size={15} />
                    </div>
                  {/if}
                </div>
    ...
    <div class="p-6 border-t border-zinc-900 bg-zinc-950 z-10 shrink-0">
      <div class="max-w-4xl mx-auto">
        ...
        <p class="text-[10px] text-zinc-650 mt-2 text-center select-none">
          Inférence sécurisée de niveau entreprise • Les données de transactions restent confidentielles
        </p>
      </div>
    </div>
  </main>
